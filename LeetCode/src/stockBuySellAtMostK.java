...
public class stockBuySellAtMostK {
    ...
    // O(kn)
    public int maxProfit_v1(int K, int[] prices) {
        int n = prices.length;
        if(n == 0) return 0;
        int[][] maxProf = new int[K+1][n]; // maxProf[i]: max profit till i
        ...
        for(int k = 1; k <= K; k++)
        {
            int min = prices[0];
            for(int i = 1; i < n; i++) { // sell
                maxProf[k][i] = maxProf[k][i-1]; // don't sell at i
                min = Math.min(min, prices[i]-maxProf[k-1][i-1]); // buy at j < i.
                maxProf[k][i] = Math.max(maxProf[k][i], prices[i]-min);
            }
        }
        return maxProf[K][n-1];
    }
